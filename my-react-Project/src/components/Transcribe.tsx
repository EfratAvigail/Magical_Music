...
const Transcribe = () => {
  ...
  const transcribeAudio = async () => {
    ...
    try {
      ...
      const transcriptionResponse = await axios.post(`https://magical-music.onrender.com/api/Transcription/transcribe`, formData, {
        headers: {
  ...
  const sendEmail = async () => {
    ...
    try {
      ...
      // Call the email API
      const response = await fetch(`https://magical-music.onrender.com/api/Email/send`, {
        method: "POST",
        headers: {
...
const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
  if (e.target.files && e.target.files[0]) {
    ...
    try {
      ...
      const response = await axios.post(`https://magical-music.onrender.com/api/Transcription/transcribe`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
